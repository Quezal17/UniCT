/*
 * distributore_bevande.c
 *
 *  Created on: 10 nov 2019
 *      Author: Simone
 */

//Note: manca gestione led rosso
...
float costo_bevande[] = { 0.35, 0.40, 0.45};
int serbatoio_monete[] = { 10, 10, 10};
float taglio_resto[] = { 0.20, 0.10, 0.05};
float resto=0;
int red_led_on = 0;
int yellow_led_on = 0;
...
int main(int argc, char* argv[]){
	...
	for(;;){
        ...
        case ST_CONFIG:
        	...
        	DISPLAY_puts(0,bevande[bevanda_selezionata]);
        	sprintf(str,"%02d",(int)(importo*10));
        	DISPLAY_puts(2,str);
        	DISPLAY_dp(2,1);
            break;
        case ST_EROGAZIONE:
        ...
        case ST_RESTO:
        	...
        	DISPLAY_dp(2,0);
        	resto=importo-costo_bevande[bevanda_selezionata];
        	for(int i=0;i<3;i++)
        		while(resto-taglio_resto[i]>=0 && serbatoio_monete[i]>0)
        		{
        			resto-=taglio_resto[i];
        			serbatoio_monete[i]--;
        			sprintf(str,"%02d",(int)(taglio_resto[i]*100));
        			DISPLAY_puts(2,str);
        			delay_ms(3000);
        		}
        	importo=0;
